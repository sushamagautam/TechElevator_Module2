...
/********************************************************************************************************************/
--Example from Jasmine's question
/********************************************************************************************************************/
SELECT * FROM city WHERE countrycode = 'GBR' AND district IN ('Wales', 'Scotland');  //or do as below for same result

SELECT * FROM city WHERE countrycode = 'GBR' AND (district = 'Wales' or district = 'Scotland');
/********************************************************************************************************************/
...
SELECT avg(lifeexpectancy) AS averageLE FROM country;
...
// ************ METHOD 1 ***************/
SELECT surfacearea FROM country ORDER BY surfacearea LIMIT 1;

// ************ METHOD 2 ***************/
SELECT MIN(surfacearea) FROM country;

// ************ A SUB SELECT APPROACH TO METHOD #1 ***********/ ----> SUB QUERRY METHOD *****/
...
//Try the line below including aggregate function
SELECT name, surfacearea FROM country GROUP BY surfacearea LIMIT 10;

-- The number of countries who declared independence in 1991

/* COUNT(*) explicitly shows we intend to count the number of rows */
...
SELECT language, COUNT(countrycode) as countriesSpoken FROM countrylanguage GROUP BY language ORDER BY countriesSpoken DESC; 
...
SELECT district, SUM(population) AS pop FROM city WHERE countrycode = 'USA' GROUP BY district ORDER BY district;

-- The average population of cities in each state in the USA ordered by state name

SELECT district, AVG(population) AS pop FROM city WHERE countrycode = 'USA' GROUP BY district ORDER BY district;
...
-- Bonus for Dennis -- Where under a head of state and speaks Welsh
