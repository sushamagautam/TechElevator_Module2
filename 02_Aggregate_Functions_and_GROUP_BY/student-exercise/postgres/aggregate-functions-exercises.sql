...
SELECT name, (gnp * 1000000)/population as gnp_per_capita FROM country WHERE population != 0 ORDER BY (gnp * 1000000)/population DESC;
...
SELECT MIN(population) FROM country WHERE population != 0;   --*************** IS NOT Null didn't work??
...
SELECT continent, SUM(surfacearea) as sum_surface_area FROM country GROUP BY continent ORDER BY sum_surface_area DESC;
...
SELECT name, (gnp - gnpold) as difference_gnp, ABS(gnp - gnpold) as absolute_value_gnp 
        FROM country
