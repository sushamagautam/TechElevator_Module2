...
--or I can also use the SUM(1) trick!

SELECT SUM(1) FROM site WHERE utilities = true;

-- What if ... I wanted both values in one result set ...

SELECT 
        COUNT(*) AS camp_sites,
        SUM( CASE WHEN utilities = True THEN 1 ELSE 0 END) AS Has_Utilities,
        SUM( CASE WHEN utilities = False THEN 1 ELSE 0 END) AS No_Utilities
FROM
        site;
...
SELECT * FROM reservation WHERE from_date > '2021-02-01' and from_date < '2021-02-28';
...
SELECT COUNT(*), from_date FROM reservation GROUP BY from_date ORDER BY from_date;