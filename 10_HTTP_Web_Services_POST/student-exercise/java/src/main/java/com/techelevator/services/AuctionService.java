...
public class AuctionService {
    ...
    public Auction update(String auctionString) {
        Auction newerAuction = makeAuction(auctionString);
        if(newerAuction == null) {
        	 return null;
        }
       
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<Auction> entity = new HttpEntity<>(newerAuction, headers);
        
        try {
        restTemplate.put(API_URL + "/" + newerAuction.getId(), entity);
        }
        catch(RestClientResponseException rcrex) {
        	return null;
        }
