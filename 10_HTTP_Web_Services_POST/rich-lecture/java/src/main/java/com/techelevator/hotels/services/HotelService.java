...
public class HotelService {
   ...
   * @return Reservation
   */
  public Reservation addReservation(String newReservationCSV) {
    //Implement adding reservation method
	  
	  
	Reservation newReservation = this.makeReservation(newReservationCSV);  
	
	if(newReservation == null) {	  
		return null;
	}

	// Define a request headers object
	...
	try {
		...
		newReservation = restTemplate.postForObject(BASE_URL + "hotels/" + newReservation.getHotelID() + 
				"/reservations", entity, Reservation.class); 
	}
	catch(RestClientResponseException rcrEx) {
		console.printError(rcrEx.getRawStatusCode() +":"+rcrEx.getStatusText() + " - " +  rcrEx.getMessage());
	}
	catch(ResourceAccessException raEx) {
		console.printError(raEx.getMessage());
	}
   ...
   * @return
   */
  public Reservation updateReservation(String updatedReservationCSV) {
    // Implement update method
	  Reservation updateReservation = this.makeReservation(updatedReservationCSV); 
	  
	  if(updateReservation == null) {
		  return null;
	  }
	  
	  // PATTERN FOR CREATING A REQUEST WITH HEADERS AND BODY
	  // -----------------------------------------------------------------------------
	  HttpHeaders headers = new HttpHeaders();
	  headers.setContentType(MediaType.APPLICATION_JSON);
	  
	  HttpEntity<Reservation> entity = new HttpEntity<>(updateReservation, headers);
	  // -----------------------------------------------------------------------------
	  
	  try {
		  restTemplate.put(BASE_URL + "reservations/" + updateReservation.getId(), entity);
	  }catch(RestClientResponseException rcrEx) {
		  console.printError(rcrEx.getRawStatusCode() + ":" + rcrEx.getStatusText());
	  }catch(ResourceAccessException raEx) {
		  console.printError(raEx.getMessage());
	  }
  ...
  private Reservation makeReservation(String CSV) {
	  ...
	  // TODO: Review how this is working: Debug Step Through
